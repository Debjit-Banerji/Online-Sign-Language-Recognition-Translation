...
def phase1_train_encoders(
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            for step, (frames, keypts, tgt_padded, tgt_lens) in enumerate(
                tqdm(val_loader, desc=f"Phase 1 ep {ep}", total=min(1000, len(val_loader)))
            ):
                if step >= 1000:
                    break
        ...
        n_val = min(1000, len(val_loader))
        print(f"  Phase 1 epoch {ep} | avg train CTC loss: {running_loss/n:.4f} | avg val CTC loss: {running_val_loss/n_val:.4f}")

        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase1_latest.pth")
        ...
        if val_loader is not None:
            ...
            if (running_val_loss/n_val) < best_loss:
                best_loss = running_val_loss/n_val
                save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase1_best_loss.pth")
                print(f"  ✓ New best Phase1 Loss={best_loss:.4f}")
...
def phase2_train_temporal_and_fusion(
    ...
    if use_pretrained:
        model = load_checkpoint("checkpoints_orig_run2/phase1_best_loss.pth", model)
        
    if use_latest:
        model = load_checkpoint("checkpoints_orig_run2/phase2_latest.pth", model)
        ctc_aux_head = load_checkpoint("checkpoints_orig_run2/ctc_aux_head_latest.pth", ctc_aux_head)
    ...
    for ep in range(start_ep, epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase2_latest.pth")
        torch.save(ctc_aux_head.state_dict(), "checkpoints_orig_run2/ctc_aux_head_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase2_best_wer.pth")
                torch.save(ctc_aux_head.state_dict(), "checkpoints_orig_run2/ctc_aux_head_best_wer.pth")
...
#         with torch.no_grad():
#             logits = model.translator(memory, dec_input)   # (B, L-1, V)
#             probs  = torch.softmax(logits, dim=-1)

#             # sample one token per position — keep as tensor
#             B, T, V = probs.shape
#             sampled = torch.multinomial(
#                 probs.reshape(-1, V), num_samples=1
#             ).reshape(B, T)                                # (B, T) — stays tensor

#         # log-probs WITH grad (memory is detached, grad flows through translator)
#         log_probs_full = torch.log_softmax(
#             model.translator(memory, dec_input), dim=-1
#         )                                                  # (B, T, V)
#         chosen_lp = log_probs_full.gather(
#             -1, sampled.unsqueeze(-1)                      # (B, T, 1) — tensor ✓
#         ).squeeze(-1)                                      # (B, T)

#         # WER reward per sequence
#         rewards = []
#         for b in range(B):
#             hyp = sampled[b].tolist()
...
#         all_log_probs.append(chosen_lp)
#         all_rewards.append(rewards)

#     # Baseline = mean reward across samples (reduces variance)
#     # Shape: (num_samples, B)
#     # reward_tensor = torch.tensor(all_rewards, dtype=torch.float32,
#                                 #  device=memory.device)     # (S, B)
#     reward_tensor = (reward_tensor - reward_tensor.mean()) / (reward_tensor.std() + 1e-6) # As suggested by CLAUDE, to stabilize RL Training.
#     baseline = reward_tensor.mean(dim=0, keepdim=True)    # (1, B)
#     advantage = reward_tensor - baseline                   # (S, B)

#     loss = torch.tensor(0.0, device=memory.device)
#     for i in range(num_samples):
#         adv = advantage[i].unsqueeze(-1)                   # (B, 1)
...
def phase3_joint_finetune(
    ...
    if use_pretrained:
        model = load_checkpoint("checkpoints_orig_run2/phase2_best_wer.pth", model)
        ctc_aux_head = load_checkpoint("checkpoints_orig_run2/ctc_aux_head_best_wer.pth", ctc_aux_head)
    
    if use_latest:
        model = load_checkpoint("checkpoints_orig_run2/phase3_latest.pth", model)
        ctc_aux_head = load_checkpoint("checkpoints_orig_run2/ctc_aux_head_phase3_latest.pth", ctc_aux_head)
    ...
    for ep in range(start_ep, epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase3_latest.pth")
        torch.save(ctc_aux_head.state_dict(), "checkpoints_orig_run2/ctc_aux_head_phase3_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints_orig_run2/phase3_best_wer.pth")
                # Add this so the hybrid decoder works at test time!
                if ctc_aux_head is not None:
                    torch.save(ctc_aux_head.state_dict(), "checkpoints_orig_run2/ctc_aux_head_phase3_best.pth")
...
# Utility to freeze/unfreeze parameters
# def set_requires_grad(model, pattern, requires_grad):
#     for name, p in model.named_parameters():
#         if pattern in name:
#             p.requires_grad = requires_grad

# # PHASED TRAINING
# def phase1_train_encoders(model, dataloader, device, epochs=3):
#     """
#     Train pose encoder (GCN) and adapter parameters while keeping CLIP frozen.
#     """
#     model.to(device)
#     # Add a temporary CTC head for Phase 1 alignment
#     ctc_head = torch.nn.Linear(model.translator.d_model, model.translator.out.out_features).to(device)

#     # Only enable gradients for PoseEncoder and adapter params (adapters were set trainable earlier)
#     for name, p in model.named_parameters():
#         p.requires_grad = False
#     # enable pose encoder params
#     for p in model.pose_encoder.parameters():
#         p.requires_grad = True
#     # Disable adapters (their names contain 'sla_adapter')
#     for name, p in model.named_parameters():
#         if "sla_adapter" in name.lower():
#             p.requires_grad = False
#     # Disable adapters for visual encoder
#     for p in model.rgb_encoder.parameters():
#         p.requires_grad = False
#     # also enable rgb_proj and pose_proj
#     for p in [model.rgb_proj.parameters(), model.pose_proj.parameters()]:
#         for q in p:
#             q.requires_grad = True

#     optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, list(model.parameters()) + list(ctc_head.parameters())), lr=1e-4)
#     ctc_loss_fn = torch.nn.CTCLoss(blank=0, zero_infinity=True)

#     model.train()
#     for ep in range(epochs):
#         for frames, keypts, tgt_padded, tgt_lens in tqdm(dataloader):
#             # Forward pass
#             frames = frames.to(device).float()
#             keypts = keypts.to(device).float()
#             out = model(frames, keypts)
#             context = out['context_seq'] # (B, T_prime, Df)

#             # CTC Projection: (B, T, D) -> (T, B, Vocab)
#             logits = ctc_head(context).transpose(0, 1).log_softmax(2)

#             # Input lengths for CTC (T_prime)
#             input_lengths = torch.full((logits.size(1),), logits.size(0), dtype=torch.long).to(device)

#             # Build flattened targets required by CTCLoss from padded targets
#             tgt_lens = tgt_lens.to(device)
#             targets_list = [tgt_padded[i, :tgt_lens[i]].to(device) for i in range(tgt_padded.size(0))]
#             if len(targets_list) > 0:
#                 targets_1d = torch.cat(targets_list)
#             else:
#                 targets_1d = torch.tensor([], dtype=torch.long, device=device)

#             loss = ctc_loss_fn(logits, targets_1d, input_lengths, tgt_lens)

#             optimizer.zero_grad(); loss.backward(); optimizer.step()
#         print(f"Phase 1 Epoch {ep} Loss: {loss.item()}")

# def phase2_train_temporal_and_fusion(model, dataloader, device, epochs=3):
#     """
#     Freeze encoders, train fusion + temporal + ctl + boundary + translator (decoder)
#     """
#     model.to(device)
#     # freeze encoder params
#     for name, p in model.named_parameters():
#         if "pose_encoder" in name or "rgb_encoder" in name:
#             p.requires_grad = False
#     # enable fusion, temporal, ctl, translator
#     for name, p in model.named_parameters():
#         if any(k in name for k in ["fusion", "temporal", "ctl", "boundary", "translator", "rgb_proj", "pose_proj"]):
#             p.requires_grad = True
#         else:
#             # keep other adapter params frozen for now
#             pass

#     trainable = [p for p in model.parameters() if p.requires_grad]
#     opt = optim.AdamW(trainable, lr=3e-4)
#     ce = nn.CrossEntropyLoss()
#     model.train()
#     for ep in range(epochs):
#         for frames, keypts, tgt, tgt_lens in tqdm(dataloader):
#             frames = frames.to(device).float()
#             keypts = keypts.to(device).float()
#             tgt = tgt.to(device).long()
#             #out = model(frames, keypts, tgt_tokens=tgt.unsqueeze(0))
#             # Shift targets right for teacher forcing #added
#             out = model(frames, keypts, tgt_tokens=tgt[:, :-1]) #added: to remove the eos token from the inp9ut. 
#             logits = out['logits']
#             if logits is not None:
#                 # token-wise CE with teacher forcing for simplicity
#                 B, T, V = logits.shape
#                 # shift targets accordingly etc. (omitted for brevity)
#                 #loss = ce(logits.view(-1, V), tgt.view(-1))
#                 loss = ce(logits.view(-1, V), tgt[:, 1:].reshape(-1)) #added: to shift the target for teacher forcing (predict next token)
#             else:
#                 loss = torch.tensor(0.0)
#             # CTL loss
#             ctl_logits = out['ctl_logits']
#             # create dummy ctllabel as sum tokens in tgt // for demo only
#             ctl_label = torch.randint(0, ctl_logits.shape[-1], (ctl_logits.shape[0],), device=ctl_logits.device)
#             ctl_loss = ce(ctl_logits, ctl_label)
#             total_loss = loss + 0.2 * ctl_loss
#             total_loss.backward()
#             opt.step()
#             opt.zero_grad()
#         print("Phase 2 Epoch", ep, "loss", total_loss.item())

# def phase3_joint_finetune(model, dataloader, device, epochs=2):
#     """
#     Jointly fine-tune small subset: adapters (LoRA/Adapter), fusion, temporal, decoder
#     """
#     model.to(device)
#     # unfreeze adapters + fusion + temporal + translator
#     for name, p in model.named_parameters():
#         if "sla_adapter" in name or any(k in name for k in ["fusion", "temporal", "ctl", "boundary", "translator", "rgb_proj", "pose_proj"]):
#             p.requires_grad = True
#         else:
#             p.requires_grad = False
#     trainable = [p for p in model.parameters() if p.requires_grad]
#     opt = optim.AdamW(trainable, lr=1e-5)
#     ce = nn.CrossEntropyLoss()
#     model.train()
#     for ep in range(epochs):
#         for frames, keypts, tgt, tgt_lens in tqdm(dataloader):
...
#             tgt = tgt.to(device).long()
#             #out = model(frames, keypts, tgt_tokens=tgt.unsqueeze(0))
#             out = model(frames, keypts, tgt_tokens=tgt[:, :-1]) #added
#             logits = out['logits']
#             if logits is not None:
#                 B, T, V = logits.shape
#                 #loss = ce(logits.view(-1, V), tgt.view(-1))
#                 loss = ce(logits.view(-1, V), tgt[:, 1:].reshape(-1)) #added
#             else:
#                 loss = torch.tensor(0.0)
#             ctl_logits = out['ctl_logits']
#             ctl_label = torch.randint(0, ctl_logits.shape[-1], (ctl_logits.shape[0],), device=ctl_logits.device)
#             ctl_loss = ce(ctl_logits, ctl_label)
#             total_loss = loss + 0.1 * ctl_loss
#             total_loss.backward()
#             opt.step()
#             opt.zero_grad()
#         print("Phase 3 Epoch", ep, "joint loss", total_loss.item())

# def validate(model, dataloader, device, phase_name, vocab_inv):
#     model.eval()
#     correct = 0
#     total = 0
    
#     print(f"\n--- Running Validation for {phase_name} ---")
#     with torch.no_grad():
#         for frames, keypts, tgt, tgt_lens in tqdm(dataloader, desc="Validation Batches"):
#             frames, keypts = frames.to(device), keypts.to(device)
#             tgt = tgt.to(device)
            
#             out = model(frames, keypts)
            
#             # Logic for Translator-based phases
#             if out['logits'] is not None:
#                 # Greedy argmax over the vocab dimension
#                 preds = torch.argmax(out['logits'], dim=-1) # (B, T)
                
#                 # Compare the first sequence in the batch
#                 # We check if the predicted indices match the target indices
#                 if torch.equal(preds[0], tgt[0]):
#                     correct += 1
            
#             total += 1
            
#     acc = (correct / total) * 100 if total > 0 else 0
#     print(f"{phase_name} Validation Accuracy: {acc:.2f}%\n")
#     model.train()
...
def run_demo():
    ...
    train_dataset = Phoenix14TDataset(root_dir=root_path, split="train", is_training=True)
    os.makedirs("checkpoints_orig_run2", exist_ok=True)

    # 2. 🚨 CRITICAL: Load saved vocab if it exists so IDs don't scramble 🚨
    if os.path.exists("checkpoints_orig_run2/vocab.json"):
        print("Loading saved vocab from disk to prevent ID scrambling...")
        with open("checkpoints_orig_run2/vocab.json", "r") as f:
            vocab = json.load(f)
        ...
        vocab = train_dataset.vocab
        with open("checkpoints_orig_run2/vocab.json", "w") as f:
            json.dump(vocab, f)
    ...
    if is_run_phase2:
        _ = phase2_train_temporal_and_fusion(
            model, train_loader, device,
            vocab=vocab, vocab_inv=vocab_inv, val_loader=val_loader,
            epochs=25, accum_steps=4, eval_every=200,
            label_smoothing=0.1, use_pretrained= False, use_latest = True
        )
        torch.cuda.empty_cache(); gc.collect()
    else:
        ctc_aux_head = None
...
#     )
    ...
    if os.path.exists("checkpoints_orig_run2/phase3_best_wer.pth"):
        print("Loading Phase 3 best weights...")
        model = load_checkpoint("checkpoints_orig_run2/phase3_best_wer.pth", model)
    elif os.path.exists("checkpoints_orig_run2/phase2_best_wer.pth"):
        print("Loading Phase 2 best weights...")
        model = load_checkpoint("checkpoints_orig_run2/phase2_best_wer.pth", model)
    else:
    ...
    if os.path.exists("checkpoints_orig_run2/ctc_aux_head_phase3_best.pth"):
        print("Loading Phase 3 CTC auxiliary head...")
        ctc_aux_head.load_state_dict(torch.load("checkpoints_orig_run2/ctc_aux_head_phase3_best.pth", map_location=device))
        ctc_aux_head.eval()
    elif os.path.exists("checkpoints_orig_run2/ctc_aux_head_best_wer.pth"):
        print("Loading Phase 2 CTC auxiliary head...")
        ctc_aux_head.load_state_dict(torch.load("checkpoints_orig_run2/ctc_aux_head_best_wer.pth", map_location=device))
        ctc_aux_head.eval()
    else:
