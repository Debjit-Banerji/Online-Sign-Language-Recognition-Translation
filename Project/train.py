...
def phase1_train_encoders(
    ...
    for ep in range(epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints/phase1_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints/phase1_best_wer.pth")
...
def phase2_train_temporal_and_fusion(
    ...
    """
    if use_pretrained:
        model = load_checkpoint("checkpoints/phase1_latest.pth", model)
    ...
    for ep in range(epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints/phase2_latest.pth")
        torch.save(ctc_aux_head.state_dict(), "checkpoints/ctc_aux_head_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints/phase2_best_wer.pth")
                torch.save(ctc_aux_head.state_dict(), "checkpoints/ctc_aux_head_best_wer.pth")
...
def phase3_joint_finetune(
    ...
    if use_pretrained:
        ...
        vocab_size = model.translator.out.out_features
        ctc_aux_head = nn.Linear(fused_dim, vocab_size).to(device)
        ctc_aux_head = load_checkpoint("checkpoints/ctc_aux_head_best_wer.pth", ctc_aux_head)
    ...
    for ep in range(epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints/phase3_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                ...
                if ctc_aux_head is not None:
                    torch.save(ctc_aux_head.state_dict(), "checkpoints/ctc_aux_head_phase3_best.pth")
                print(f"  ✓ New best Phase3 WER={best_wer:.4f}")
...
def run_demo():
    ...
    if os.path.exists("checkpoints/vocab.json"):
    ...
    # NEW: Create and save model configuration for inference
    model_config = {
        ...
        "eos_idx": vocab.get("<eos>", 3),
        "adapter_dim": 256,
        "adapter_layers": list(range(2, 12)), # JSON can't serialize range
        "clip_frames": 4,
        "clip_spatial": 14,
    }
    with open("checkpoints/model_config.json", "w") as f:
        json.dump(model_config, f, indent=2)
    print("Model configuration saved to checkpoints/model_config.json")
...
#     )
    ...
    if os.path.exists("checkpoints/ctc_aux_head_phase3_best.pth"):
        print("Loading Phase 3 CTC auxiliary head...")
        ctc_aux_head.load_state_dict(torch.load("checkpoints/ctc_aux_head_phase3_best.pth", map_location=device))
        ctc_aux_head.eval()
    elif os.path.exists("checkpoints/ctc_aux_head_best_wer.pth"):
