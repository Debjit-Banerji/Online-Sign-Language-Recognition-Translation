...
def phase1_train_encoders(
    ...
    for ep in range(epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase1_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase1_best_wer.pth")
...
def phase2_train_temporal_and_fusion(
    ...
    """
    if use_pretrained:
        model = load_checkpoint("checkpoints_16Apr_2200/phase1_best_wer.pth", model)
    ...
    for ep in range(epochs):        
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase2_latest.pth")
        torch.save(ctc_aux_head.state_dict(), "checkpoints_16Apr_2200/ctc_aux_head_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase2_best_wer.pth")
                torch.save(ctc_aux_head.state_dict(), "checkpoints_16Apr_2200/ctc_aux_head_best_wer.pth")
...
def phase3_joint_finetune(
    ...
    """
    if use_pretrained:
        model = load_checkpoint("checkpoints_16Apr_2200/phase2_best_wer.pth", model)

        fused_dim  = model.translator.d_model
        vocab_size = model.translator.out.out_features
        ctc_aux_head = nn.Linear(fused_dim, vocab_size).to(device)
        ctc_aux_head = load_checkpoint("checkpoints_16Apr_2200/ctc_aux_head_best_wer.pth", ctc_aux_head)
    ...
    for ep in range(epochs):
        ...
        # ── SAVE LATEST ──
        save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase3_latest.pth")
        ...
        if val_loader is not None:
            ...
            if val_m["WER"] < best_wer:
                best_wer = val_m["WER"]
                save_checkpoint(model, optimizer, ep, f"checkpoints_16Apr_2200/phase3_best_wer.pth")
                # Add this so the hybrid decoder works at test time!
                if ctc_aux_head is not None:
                    torch.save(ctc_aux_head.state_dict(), "checkpoints_16Apr_2200/ctc_aux_head_phase3_best.pth")
                print(f"  ✓ New best Phase3 WER={best_wer:.4f}")
...
def run_demo():
    ...
    train_dataset = Phoenix14TDataset(root_dir=root_path, split="train", is_training=True)
    os.makedirs("checkpoints_16Apr_2200", exist_ok=True)

    # 2. 🚨 CRITICAL: Load saved vocab if it exists so IDs don't scramble 🚨
    if os.path.exists("checkpoints_16Apr_2200/vocab.json"):
        print("Loading saved vocab from disk to prevent ID scrambling...")
        with open("checkpoints_16Apr_2200/vocab.json", "r") as f:
            vocab = json.load(f)
        ...
        vocab = train_dataset.vocab
        with open("checkpoints_16Apr_2200/vocab.json", "w") as f:
            json.dump(vocab, f)
    ...
    if is_run_phase1:
        phase1_train_encoders(
            ...
            epochs=10, accum_steps=1, eval_every=400,
        )
    ...
    if is_run_phase2:
        ctc_aux_head = phase2_train_temporal_and_fusion(
            model, train_loader, device,
            vocab=vocab, vocab_inv=vocab_inv, val_loader=val_loader,
            epochs=20, accum_steps=1, eval_every=200,
    ...
    if is_run_phase3:
        phase3_joint_finetune(
            ...
            epochs=5, accum_steps=1, eval_every=200,
...
#     )
    ...
    if os.path.exists("checkpoints_16Apr_2200/phase3_best_wer.pth"):
        print("Loading Phase 3 best weights...")
        model = load_checkpoint("checkpoints_16Apr_2200/phase3_best_wer.pth", model)
    elif os.path.exists("checkpoints_16Apr_2200/phase2_best_wer.pth"):
        print("Loading Phase 2 best weights...")
        model = load_checkpoint("checkpoints_16Apr_2200/phase2_best_wer.pth", model)
    else:
    ...
    if os.path.exists("checkpoints_16Apr_2200/ctc_aux_head_phase3_best.pth"):
        print("Loading Phase 3 CTC auxiliary head...")
        ctc_aux_head.load_state_dict(torch.load("checkpoints_16Apr_2200/ctc_aux_head_phase3_best.pth", map_location=device))
        ctc_aux_head.eval()
    elif os.path.exists("checkpoints_16Apr_2200/ctc_aux_head_best_wer.pth"):
        print("Loading Phase 2 CTC auxiliary head...")
        ctc_aux_head.load_state_dict(torch.load("checkpoints_16Apr_2200/ctc_aux_head_best_wer.pth", map_location=device))
        ctc_aux_head.eval()
    else:
